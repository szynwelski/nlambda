...
instance Conditional Formula where
    iF f1 = return $ \f2 f3 -> (f1 /\ f2) \/ (not f1 /\ f3)
...
iFVariants :: (a -> [Variant b]) -> ([Variant b] -> a) -> Formula -> IO (a -> a -> a)
iFVariants toVariantList fromVariantList f = do
    result <- (ifFormulaIO f)
    return (fromMaybe (ifVariants f) result)
...
iFv :: Formula -> IO (a -> a -> Variants a)
iFv c = do
    result <- iF c
    return $ \x1 x2 -> result (variants x1) (variants x2)
...
instance Conditional b => Conditional (a -> b) where
    iF f = do
        result <- (iF f)
        return $ \f1 f2 -> \x -> result (f1 x) (f2 x)
...
--        result <- iF (f x)
--        return :
--    (\x -> (iF (f x) (just x) emptySet))
--
--    result <- iF (f
--
--
--sum $ map (\x -> (iF (f x) (just x) emptySet)) s
--
