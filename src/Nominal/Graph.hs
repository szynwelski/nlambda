...
clique :: Nominal a => Set a -> Graph a
...
simpleClique :: Nominal a => Set a -> Graph a
...
compose :: Nominal a => Graph a -> Graph a -> Graph a
...
subgraph :: Nominal a => Graph a -> Set a -> Graph a
...
weaklyConnectedComponent :: Nominal a => Graph a -> a -> Graph a
...
weaklyConnectedComponents :: Nominal a => Graph a -> Set (Graph a)
...
stronglyConnectedComponent :: Nominal a => Graph a -> a -> Graph a
...
stronglyConnectedComponents :: Nominal a => Graph a -> Set (Graph a)
...
isColoringOf :: (Nominal a, Nominal b) => (a -> b) -> Graph a -> Formula
isColoringOf c g = forAll (\(v1,v2) -> c v1 `neq` c v2) (edges g)

-- | Checks whether a graph has an equivariant k-coloring.
hasEquivariantColoring :: (Contextual a, Nominal a) => Graph a -> Int -> Formula
